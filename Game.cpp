...
void Game::Update(DX::StepTimer const& timer)
{
	...
	//m_reflection = m_Camera01.getCameraMatrix();
...
//	context->RSSetState(m_states->Wireframe());
	...
	RenderTexturePass1();
	//m_FirstRenderPass->getShaderResourceView();
	// 
	...
	//m_Camera01.RenderReflection(-5.0f);
	//m_reflection = m_Camera01.getReflectionMatrix();
	// Turn our shaders on,  set parameters
	m_reflectionShader1.EnableReflectionShader(context);
	m_reflectionShader1.SetShaderParameters(context, &m_world, &m_view, &m_projection, m_reflectTexture.Get(), m_FirstRenderPass->getShaderResourceView(), &m_reflection);
	////setup and draw cube
	//m_BasicShaderPair.EnableShader(context);
	//m_BasicShaderPair.SetShaderParameters(context, &m_world, &m_view, &m_projection, &m_Light, m_texture2.Get());
...
void Game::RenderTexturePass1()
{
	...
	/*m_reflectionShader1.EnableReflectionShader(context);
	m_reflectionShader1.SetShaderParameters(context, &m_world, &m_view, &m_projection, m_texture1.Get(), m_reflectTexture.Get(), &m_reflection);*/
	...
	m_BasicShaderPair.EnableShader(context);
	m_BasicShaderPair.SetShaderParameters(context, &m_world, &m_reflection, &m_projection, &m_Light, m_texture2.Get());
	...
	//m_Camera01.RenderReflection(-1.5f);
	//m_reflection = m_Camera01.getReflectionMatrix();
...
void Game::CreateDeviceDependentResources()
{
	...
	m_BasicModel3.InitializeBox(device, 10.0f, 0.2f, 10.0f);	//box includes dimensions

	//load and set up our Vertex and Pixel Shaders
	m_BasicShaderPair.InitStandard(device, L"light_vs.cso", L"light_ps.cso");
	m_reflectionShader1.InitializeShader(device, L"reflection_vs.cso", L"reflection_ps.cso");

	//load Textures
	CreateDDSTextureFromFile(device, L"seafloor.dds",		nullptr,	m_texture1.ReleaseAndGetAddressOf());
	CreateDDSTextureFromFile(device, L"EvilDrone_Diff.dds", nullptr,	m_texture2.ReleaseAndGetAddressOf());
	CreateDDSTextureFromFile(device, L"water.dds", nullptr, m_reflectTexture.ReleaseAndGetAddressOf());

	//Initialise Render to texture
	m_FirstRenderPass = new RenderTexture(device, 1080, 900, 1, 2);	//for our rendering, We dont use the last two properties. but.  they cant be zero and they cant be the same. 
